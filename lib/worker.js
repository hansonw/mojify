...
const FS_QUOTA = 128 * 1024 * 1024; // chi_tra + chi_sim unzipped size
const SUPER_XBR_LIMIT = 512 * 512;  // superxbr is too slow for large images
...
function initFilesystem() {
  if (fileSystem == null) {
    fileSystem = webkitRequestFileSystemSync(PERSISTENT, FS_QUOTA);
  }
}
...
async function loadLanguage(lang, index) {
  ...
  initFilesystem();
  try {
    const entry = fileSystem.root.getFile(key, {create: false});
    ...
    createDataFile(new Uint8Array(reader.result));
    console.log(`Loaded data for ${lang} from file system.`);
    return;
  } catch (err) {
    console.log(err);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = async function () {
      if (xhr.status === 200 || (xhr.status === 0 && xhr.response)) {
        ...
        createDataFile(data);
        const entry = fileSystem.root.getFile(key, {create: true});
        entry.createWriter().write(new Blob([data.buffer]));
        resolve(true);
      }
      reject(xhr);
    };
...
function desaturate(src, width, height) {
...
async function recognize(index, image, lang, options) {
  ...
  let desaturated = desaturate(image.data, width, height);
  if (width * height < SUPER_XBR_LIMIT) {
    desaturated = superxbr(desaturated, width, height);
    width *= 2;
    height *= 2;
  }
  const ptr = Module.allocate(desaturated, 'i8', Module.ALLOC_NORMAL);
  ...
  Module._free(ptr);
  console.log('%fs spent recognizing', (Date.now() - time) / 1000);
  return everything;
}
